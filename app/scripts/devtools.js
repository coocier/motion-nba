(function () {
  ...
  chrome.devtools.panels.elements.createSidebarPane("Images", (sidebar) => {
    sidebar.setPage('pages/devtools.html');
    init();

    chrome.runtime.sendMessage({cmd: 'created'});
  ...
  const eventBind = () => {
    ...
    $('#cameraDetect').off('change').on('change', (e) => {
      ...
      if(e.target.checked) {
        ...
        chrome.runtime.sendMessage({cmd: 'confidence', value: confidence });
        chrome.runtime.sendMessage({cmd: 'openCamera'});
        sensorWrap.show();

      } else {
        chrome.runtime.sendMessage({cmd: 'closeCamera'});
        sensorWrap.hide();
      }
    });
    ...
    $('#slider').off('change').on('change', (e) => {
      chrome.runtime.sendMessage({cmd: 'confidence', value: e.target.value });
    });
    ...
    $(document).off('click', '#gameListContent .list-group-item').on('click', '#gameListContent .list-group-item', (e) => {
      ...
      this.recordsInterval = setInterval(()=>{
        $('#textRecords .content').html(null);
        getMaxsid(gameId).then((maxSid) => {
          if(currentMaxSid === maxSid) return;
          currentMaxSid = maxSid;
          getLiveRecord(gameId, maxSid).then(data => {
            // data = [{
            //   guess_data: ""
            //   guess_id: "0"
            //   guess_text: ""
  ...
  const getLiveRecord = (gameId, maxSid) => {
    return $.ajax({
      type : "GET",
