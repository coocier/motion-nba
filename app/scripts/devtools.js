class devtools {
  ...
  init () {
    // this.createPanel();
    browser.storage.local.get("createInfo").then(data => {

      if(!data || !data.createInfo || !data.createInfo.isPanelCreated) {
        console.log('in')
        let createInfo = {
          name: 'devtools',
          isPanelCreated: true
        }
        browser.storage.local.set({ createInfo }).then(() => {
  ...
  destory () {
    $('#cameraDetect').off('change');
    $('#slider').off('change');
    $('#reload').off('click');
    $('#back').off('click');
    $(document).off('click', '#gameListContent .list-group-item');
  ...
  eventBind () {
    ...
    $('#cameraDetect').off('change').on('change', (e) => {
      ...
      if (e.target.checked) {
        ...
        this.sendMessageToBack({ cmd: 'setConfidence', value: confidence })
        this.sendMessageToBack({ cmd: 'openCamera' })
        sensorWrap.show()
      } else {
        this.sendMessageToBack({ cmd: 'closeCamera' })
        sensorWrap.hide()
      }
    });
    ...
    $('#slider').off('change').on('change', (e) => {
      this.sendMessageToBack({ cmd: 'setConfidence', msg: e.target.value })
    })
